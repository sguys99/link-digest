...
| 항목 | 내용 |
|---|---|
| 역할 | 개발자, 연구자, 기획자 등 지속적으로 자기개발하는 전문직 종사자 |
| 행동 패턴 | 매일 5~10개 이상의 기술 블로그, 논문, 뉴스, 유튜브 링크를 접함 |
...
| 항목 | 내용 |
|---|---|
| 테스트 유형 | 클로즈드 베타 (초대 기반) |
| 참여 인원 | 10명 |
| 대상 | 개발자, 연구자 등 자기개발에 관심 있는 전문직 종사자 |
| 기간 | 2주 |
| 플랫폼 | Vercel 배포 (Pro Plan — 동시 접속 10명 충분 수용) |

### 11.2 인프라 요구사항

| 리소스 | 사양 | 근거 |
|---|---|---|
| Vercel | Hobby 또는 Pro Plan | Serverless Functions 동시 실행, 10명 규모 충분 |
| Supabase | Free Plan | 500MB DB, 50,000 Auth MAU — 10명 테스트에 충분 |
| Resend | Free Plan | 3,000건/월 — 10명 주간 발송에 충분 |
| LLM API | 각 테스터 본인 API 키 사용 | 서비스 측 비용 없음 |

### 11.3 테스트 단계

**Phase 1: 내부 검증 (1~2일)**
- 개발자 본인이 전체 플로우를 end-to-end 검증
- PWA 설치 → 링크 공유 → 요약 확인 → 뉴스레터 수신 전 과정 확인
- iOS / Android 각각 테스트

**Phase 2: 베타 테스터 초대 (1주차)**
- 10명에게 초대 이메일 발송 (가입 링크 + 온보딩 가이드)
- 각 테스터가 본인 LLM API 키를 설정하고 서비스 사용 시작
- 일일 최소 3개 이상 링크 저장 권장

**Phase 3: 피드백 수집 (2주차)**
- 주간 뉴스레터 최소 1회 수신 후 피드백 수집
- Google Forms 또는 Notion 설문으로 정량/정성 피드백 수집

### 11.4 테스트 체크리스트

| 구분 | 항목 | 검증 방법 |
|---|---|---|
| 인증 | Google OAuth 로그인/로그아웃 | 10명 전원 정상 로그인 |
| PWA | 홈 화면 추가 + 공유 시트 노출 | iOS/Android 각 최소 3명 확인 |
| 링크 저장 | 공유 시트에서 링크 저장 | 다양한 URL 유형 (블로그, 뉴스, 유튜브) |
| AI 요약 | 요약 생성 정확도 및 속도 | 30초 이내 완료율 측정 |
| 대시보드 | 목록 조회, 읽음 토글, 삭제 | 각 기능 정상 작동 확인 |
| 뉴스레터 | 설정된 시간에 이메일 수신 | 10명 전원 수신 확인 |
| 알림 | Slack/Telegram 알림 수신 | 설정한 테스터 대상 확인 |
| 성능 | 10명 동시 사용 시 응답 속도 | API 응답 2초 이내 유지 |
